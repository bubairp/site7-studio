...
| Commerce24 configured but unreachable during catalog listing | `CommerceClient::send()` catches the Guzzle exception and converts it to `CommerceApiException`; `Commerce24MarketplaceRepository::listAvailablePackages()` catches that in turn and returns `[]` — same degrade-silently behavior as "not configured," logged via `Craft::warning()`. |
| Commerce24 configured but unreachable during `fetchPackage()` (install-time download) | The same `CommerceApiException` is instead re-thrown (wrapped as a plain `\Exception`) and does surface to the caller — a failed install-time fetch is not silently swallowed. |
