...
1. **Execution model for Phase 6/7** — synchronous CP request, a Craft queue job, or a console command the wizard shells out to? Long-running Composer/npm installs don't fit well inside a normal PHP-FPM request lifetime. **Partially resolved by Phase 6 live testing**: a single unbroken PHP process cannot both `composer require` a new plugin and then `Plugins::installPlugin()`/`enablePlugin()` it (Craft caches its Composer-plugin manifest once per process, confirmed directly) — whatever Phase 7 builds needs at least one process boundary between those two step types (a queue job requeuing itself, or a console command re-invoking itself), ruling out a naive single synchronous-request implementation whenever new plugins are involved.
