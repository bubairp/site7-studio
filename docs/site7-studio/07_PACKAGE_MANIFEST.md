...
| `version` | string | required | Semver `MAJOR.MINOR.PATCH` | `VersionManagerService::createVersion()` (the only sanctioned writer of a NEW value) |
| `author`, `description`, `category`, `tags`, `compatibility` | string/array | optional | Basic metadata | `PackageAuthoringService::updatePackage()` |
...
| `displayName`, `company`, `website`, `supportUrl`, `documentationUrl`, `license`, `pricingType`, `minimumCraftVersion`, `minimumSite7Version`, `keywords` | string/array | optional | Publishing/catalog metadata | `PackageAuthoringController::actionSaveMetadata()` |
