...
Starter-Kit-build-time:  DependencyAnalyzer (via StarterKitBuilder)
   ↓  computeClosure() — BFS over the whole-project CraftResourceRegistry graph,
...
5. Separately, `DependencyAnalyzer::computeClosure()` — used only by `StarterKitBuilder` at build time, not install time — narrows the whole-project `CraftResourceRegistry` graph to exactly what a specific manifest needs, producing ordered installation waves (`32_STARTER_KIT_SYSTEM.md`).
...
**`DependencyAnalyzer`** — `src/services/DependencyAnalyzer.php`. Method: `computeClosure()`, static helpers `buildContentItems()`/`buildFrontendItems()` (pure, unit-testable without a live Craft app — see `DependencyAnalyzerTest`).
