...
Codeception 5 + PHPUnit, `unit` suite, `UnitTester` actor. 28 test files under `plugins/site7-studio/tests/`.
...
**Genuine assertion-mismatch failures** (not environment errors — real logic/test drift):
- `tests/unit/services/import/ResourceImportValidatorTest.php` — `testFlagsUnsupportedFieldsAsWarnings`, `testFlagsAssetsFieldsAsWarning` fail.
- `tests/unit/services/import/ResourceClassifierServiceTest.php` — `testUnsupportedFieldWithNoSignalIsUnknownResource` fails (expected `'unknown-resource'`, actual `'review-required'`).
...
| `ResourceImportValidatorTest`/`ResourceClassifierServiceTest` specific assertions fail | Real drift between test expectations and current classification behavior | Needs investigation — not diagnosed further in this documentation pass |
