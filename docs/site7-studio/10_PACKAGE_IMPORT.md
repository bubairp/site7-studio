...
- **`overwriteConflicts` option**: `MarketplaceService::updatePackage()` (the Marketplace "Update" button) always passes `true` — this is the one place a wholesale package-source overwrite is expected and intentional.
