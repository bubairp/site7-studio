...
Site7Studio::attachEventHandlers()
   ↓
   1. yii\base\Event::on(UrlManager::class, EVENT_REGISTER_CP_URL_RULES, ...) — CP routes
   ↓
   2. Registers PatternMatrixBundle + injects window.site7Studio on every CP page
   ↓
   3. Adds "Save as Template" alt-action to the Entry edit Save dropdown
   ↓
...
5. CP routes are registered explicitly (see `28_CONTROLLERS_AND_ROUTES.md` for the full list) — every other action is reached via Craft's default `site7-studio/<controller-id>/<action-id>` resolution.
