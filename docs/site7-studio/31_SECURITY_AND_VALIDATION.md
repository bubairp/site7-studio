...
DESTRUCTIVE-OP GATING  deletePackage()/detachPackage() — Dev-Mode-gated (with a self-captured-
                       Template exception for deletePackage only)
...
VERSION INTEGRITY      site7_package_versions unique (packageId, version); recordVersion()
                       dedup-safe; VersionManagerService bump-base-off-history fix prevents
                       post-rollback collisions
