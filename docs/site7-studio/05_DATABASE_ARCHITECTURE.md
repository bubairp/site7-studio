...
Explain the complete SITE7 Studio database schema, how the 15 tables relate, and which service owns writes to each. This is the narrative companion to `37_DATABASE_TABLE_REFERENCE.md`'s flat lookup table — read this document for relationships and lifecycle, that one for a quick per-table lookup.

## 2. What It Does

15 tables (all prefixed `site7_`, Craft's `{{%...}}` table-prefix convention applies to all), created across 15 timestamped migrations + one no-op `Install.php` stub.
