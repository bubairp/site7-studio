...
Standard Craft CP controller conventions apply (`requireCpRequest()`/`requirePostRequest()`/permission checks) — not independently re-verified per-action in this research pass; assume standard Craft controller security unless a specific action's code states otherwise.
