...
| no | no | `RESULT_CONFLICT` — both sides changed. `classify()` (`PackageUpdatePlanner.php:145-149`) returns `RESULT_CONFLICT` unconditionally here, **regardless of whether LIVE == INCOMING** (the code's own comment says so explicitly) — a forward update reports a conflict even if the developer's edit happens to match the incoming content byte-for-byte. Only `Rollback` treats a LIVE==INCOMING match as already-satisfied, and only as a post-processing step outside `classify()` (see §10 and `20_ROLLBACK.md`). |
...
```mermaid
flowchart TD
    A["compare LIVE vs BASELINE"] -->|"LIVE == BASELINE"| B["compare INCOMING vs BASELINE"]
    A -->|"LIVE != BASELINE"| C["compare INCOMING vs BASELINE"]
    B -->|"INCOMING == BASELINE"| U["RESULT_UNCHANGED — nothing to do"]
    B -->|"INCOMING != BASELINE"| S["RESULT_SAFE_UPDATE — apply incoming"]
    C -->|"INCOMING == BASELINE"| L["RESULT_LOCAL_MODIFICATION — leave alone"]
    C -->|"INCOMING != BASELINE"| X["RESULT_CONFLICT — unconditionally,\neven if LIVE == INCOMING"]
...
**No content-equal short-circuit on forward updates**: if LIVE already equals INCOMING (both changed, but landed on the same content — e.g. a developer manually applied the same fix the package update would have made), `classify()` still returns `RESULT_CONFLICT` — it does not special-case this. A forward `Update` will report a conflict and leave the file untouched even though applying it would lose nothing. This short-circuit genuinely exists in the codebase, but only inside `PackageRollbackService::reconcileAlreadyMatchingConflicts()` (`src/services/publishing/PackageRollbackService.php:152-180`), run as a post-processing pass **after** `classify()`, and scoped to `Rollback` only. That class's own docblock states this is deliberate: the refinement isn't inside `classify()` because "that function is shared with forward updates" and the condition doesn't apply there. See `20_ROLLBACK.md` §5.
...
| Developer edited file, package changed it to the SAME content | `RESULT_CONFLICT` on a forward Update (file untouched, reported to the developer) — only `Rollback`'s separate reconciliation pass treats this as already-satisfied |
