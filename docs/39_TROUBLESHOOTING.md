...
### 14. "Add Section"/"Insert Pattern" buttons appear on a field nested inside a site7Components block (e.g. a "CTA Banner" block's own "CTA Button" field), and/or that nested field's own native "+" control is missing — RESOLVED
**Likely cause (was)**: `injectButton()` located the site7Components field's own `.buttons` container with `.find('.buttons').first()`, which searches all descendants at any depth — a Matrix-type field nested inside one of site7Components' own blocks has its own `.buttons` too, and (since Craft renders a field's blocks/entries before its own `.buttons`) that nested one sorts earlier in DOM order, so `.first()` silently grabbed the wrong one. Separately, the CSS hiding native controls used a descendant combinator (`.site7-matrix-override .buttons > *`) instead of a child combinator, which also hid the nested field's own native control. Fixed 2026-08-18 — see `44_CONTENT_BROWSER_MATRIX_INJECTION.md` §11B.
**Inspect**: if a similar symptom recurs, check whether any `.find()`/descendant-combinator-CSS logic in `pattern-matrix.js` that's meant to target "this field's own X" could instead match something nested inside one of this field's own blocks/entries.
**Safe steps**: same cache-clearing note as item 13 above.
