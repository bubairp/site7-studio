...
- **Registries** (`src/registries/*.php`): `MemoryPackageRegistry` — the in-memory `PackageRegistryInterface` implementation `PackageDiscovery` populates during a scan, before `PackageManagerService::discoverPackages()` persists its contents to `PackageRecord` rows (see `06_PACKAGE_ARCHITECTURE.md`).
