...
```mermaid
flowchart LR
    A["Create/Author\n08"] --> M["Manifest written\n07"]
    B["Import\n10, 14, 15"] --> M
    M --> V1["First Version\n17"]
    V1 --> BX["Build/Export\n09"]
    BX --> P["Publish (optional)\n23"]
    BX --> I["Install\n11"]
    P --> I
    I --> BL["Baseline recorded\n16"]
    BL --> E["developer edits installed files"]
    E --> SY["Sync From Source\n18"]
    SY --> UP["Update\n19"]
    UP --> RB["Rollback (as needed)\n20"]
    RB --> UP
    UP --> UN["Uninstall / Delete / Detach\n12"]
```
...
**`PackageDiscovery`** — `src/services/engine/PackageDiscovery.php`. Scans a directory for `manifest.json`-bearing subdirectories and populates an in-memory `PackageRegistryInterface` registry (`MemoryPackageRegistry`) — it does NOT touch the database itself. `PackageManagerService::discoverPackages()` then persists that registry's contents into `PackageRecord` rows and clears it — DB persistence is one layer up from `PackageDiscovery`.
