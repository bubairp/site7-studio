...
│   ├── registries/                   # MemoryPackageRegistry - in-memory PackageRegistryInterface impl used by PackageDiscovery
...
| `repositories/` (root) | Table-scoped CRUD for source-tracking tables, AND general package CRUD (`PackageRepository`, which lives here too, not elsewhere) | Marketplace file/HTTP-backed repositories — those go in `repositories/marketplace/` |
